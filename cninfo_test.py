...
class Cninfo(object):
    ...
    def sendWX(self):
        ...
        print(self.content)
        self.file = open('/Users/liyungui/cn_logg.txt', 'a+')
    ...
    def build_content(self):
        if self.content_list.__len__() > 0:
            self.content += self.group_title + ' 有新公告 ' + str(self.content_list.__len__()) + ';'
            self.content_list.clear()
        else:
            self.content += self.group_title + ' 没有新公告 ;'


def main(obj, keywords):
    obj.content = datetime.datetime.now().strftime('%Y-%m-%d %H:%M %S') + ' '
    ...
    # t = Timer(10, main, (obj, keywords,))
    t = Timer(30 * 60, main, (obj, keywords,))
    t.start()
